namespace MiJuegoRPG.Core.Combate.Orden
{
    ...
    public readonly struct ActionOrderKey : IComparable<ActionOrderKey>
    {
        ...
        {
            int c;
            if ((c = ScheduledTick.CompareTo(o.ScheduledTick)) != 0)
            {
                return c;
            }

            if ((c = PhaseOrdinal.CompareTo(o.PhaseOrdinal)) != 0)
            {
                return c;
            }

            if ((c = PriorityTier.CompareTo(o.PriorityTier)) != 0)
            {
                return c;
            }

            if ((c = o.SpeedScore.CompareTo(SpeedScore)) != 0)
            {
                return c; // mayor velocidad primero
            }

            if ((c = Sequence.CompareTo(o.Sequence)) != 0)
            {
                return c;
            }
