...
namespace MiJuegoRPG.Motor.Menus
{
    ...
    public class MenuAdmin
    {
        ...
        private static string NormalizarRarezaTextoLocal(string s)
        {
            ...
            if (string.Equals(s, "Comun", StringComparison.OrdinalIgnoreCase) || string.Equals(s, "Común", StringComparison.OrdinalIgnoreCase))
                return "Normal"; // mapeo a enum existente
                ...
                switch (op)
                {
                    case "1":
                        ...
                        if (!string.IsNullOrWhiteSpace(id))
                            juego.TeletransportarASector(id);
                        break;
                    case "2":
                        ...
                        {
                            juego.Ui.WriteLine("Valor inválido.");
                        }
                    ...
                    case "3":
                        ...
                        {
                            juego.Ui.WriteLine("Facción vacía.");
                            break;
                        }
                    ...
                    case "4":
                        ...
                        {
                            juego.Ui.WriteLine("No hay jugador cargado.");
                            break;
                        }
                        juego.Ui.WriteLine($"[REP] Global: {juego.Jugador.Reputacion}");
                        if (juego.Jugador.ReputacionesFaccion.Count == 0)
                        {
                            juego.Ui.WriteLine("[REP] Sin reputaciones de facción.");
                        }
                        else
                        {
                            foreach (var kv in juego.Jugador.ReputacionesFaccion.OrderBy(k => k.Key))
                                juego.Ui.WriteLine($"[REP] {kv.Key}: {kv.Value}");
                        }
                    ...
                    case "6":
                        if (juego.Jugador == null)
                        {
                            juego.Ui.WriteLine("No hay jugador.");
                            break;
                        }
                    ...
                    case "7":
                            ...
                            if (ModificarAtributosBatch(juego.Jugador, entrada))
                            {
                                ...
                                try
                                {
                                    if (juego.ClaseService.Evaluar(juego.Jugador))
                                        juego.Ui.WriteLine("[CLASES] Se han desbloqueado nuevas clas\nes tras el cambio de atributo.");
                                }
                                catch { }
                            }
                            break;
                        ...
                        if (double.TryParse(dat.Replace("+", string.Empty), System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out var deltaAtr))
                        {
                            ...
                            {
                                juego.Ui.WriteLine("Atributo desconocido.");
                            }
                        }
                        else
                        {
        ...
        private void DarObjetoPorNombre()
        {
            ...
            try
            {
                MiJuegoRPG.Motor.GeneradorObjetos.CargarEquipoAuto();
            }
            catch { }
            try
            {
                if (MiJuegoRPG.Objetos.GestorMateriales.MaterialesDisponibles.Count == 0)
                    MiJuegoRPG.Objetos.GestorMateriales.CargarMateriales("materiales.json");
            }
            catch { }
            try
            {
                if (MiJuegoRPG.Objetos.GestorPociones.PocionesDisponibles.Count == 0)
                    MiJuegoRPG.Objetos.GestorPociones.CargarPociones("pociones.json");
            }
            catch { }
            ...
            if (idx > 0)
            {
                prefijo = entrada.Substring(0, idx).Trim().ToLowerInvariant();
            ...
            try
            {
                ...
                foreach (var (campo, tipo) in campos)
                {
                    ...
                    {
                        var t = it.GetType();
                        var pn = t.GetProperty("Nombre");
                        var v = pn?.GetValue(it)?.ToString();
            ...
            try
            {
                if (prefijo == null || prefijo == "material")
                    AgregarMatches(MiJuegoRPG.Objetos.GestorMateriales.MaterialesDisponibles.ConvertAll(m => m.Nombre), "material");
            }
            catch { }
            try
            {
                if (prefijo == null || prefijo == "pocion")
                    AgregarMatches(MiJuegoRPG.Objetos.GestorPociones.PocionesDisponibles.ConvertAll(p => p.Nombre), "pocion");
            }
            catch { }
            ...
            const int MAX = 25;
            for (int i = 0; i < Math.Min(MAX, lista.Count); i++)
                juego.Ui.WriteLine($"{i + 1}. [{lista[i].tipo}] {lista[i].nombre}");
            if (lista.Count > MAX)
                juego.Ui.WriteLine($"... (+{lista.Count - MAX} más)");
            ...
            {
                if (idxSel >= 1 && idxSel <= Math.Min(MAX, lista.Count))
                    elegido = lista[idxSel - 1];
            }
            else
            {
                elegido = lista.FirstOrDefault(t => string.Equals(t.nombre, pick, StringComparison.OrdinalIgnoreCase));
                if (elegido?.nombreSel == null)
                    elegido = lista.FirstOrDefault(t => t.nombre.IndexOf(pick, StringComparison.OrdinalIgnoreCase) >= 0);
            }
            if (elegido == null)
            {
            ...
            try
            {
                ...
                switch (tipoSel)
                {
                    case "arma":
                        ...
                        break;
                    case "armadura":
                        obj = CrearArmaduraDesdeNombre(nombreSel);
                        break;
                    case "accesorio":
                        obj = CrearAccesorioDesdeNombre(nombreSel);
                        break;
                    case "botas":
                        obj = CrearBotasDesdeNombre(nombreSel);
                        break;
                    case "casco":
                        obj = CrearCascoDesdeNombre(nombreSel);
                        break;
                    case "cinturon":
                        obj = CrearCinturonDesdeNombre(nombreSel);
                        break;
                    case "collar":
                        obj = CrearCollarDesdeNombre(nombreSel);
                        break;
                    case "pantalon":
                        obj = CrearPantalonDesdeNombre(nombreSel);
                        break;
                    case "material":
                        var mat = MiJuegoRPG.Objetos.GestorMateriales.BuscarMaterialPorNombre(nombreSel);
                        if (mat != null)
                        {
                            pj.Inventario.Agregar(mat, 1);
                            juego.Ui.WriteLine($"Entregado material '{nombreSel}'.");
                        }
                        else
                        {
                            juego.Ui.WriteLine("Material no encontrado en catálogo.");
                        }
                    ...
                    case "pocion":
                        ...
                        if (poc != null)
                        {
                            pj.Inventario.AgregarObjeto(poc, 1, pj);
                            juego.Ui.WriteLine($"Entregada poción '{nombreSel}'.");
                        }
                        else
                        {
                            juego.Ui.WriteLine("Poción no encontrada en catálogo.");
                        }

                        return;
                    default:
                        juego.Ui.WriteLine("Tipo no soportado en esta versión.");
                        return;
                }
                if (obj == null)
                {
                    juego.Ui.WriteLine("No se pudo instanciar el objeto.");
                    return;
                }
        ...
        private void EntregarSetGMCompleto()
        {
            ...
            foreach (var (tipo, nombre, creador) in piezas)
            {
                try
                {
                    ...
                    if (obj == null)
                    {
                        juego.Ui.WriteLine($"[GM] No se encontró '{nombre}' ({tipo}) en el catálogo.");
                        continue;
                    }
                    pj.Inventario.AgregarObjeto(obj, 1, pj);
                    entregados.Add(obj);
            ...
            {
                juego.Ui.WriteLine("[GM] No se entregó ninguna pieza.");
                return;
            }
        ...
        private MiJuegoRPG.Objetos.Arma? CrearArmaDesdeNombre(string nombre)
        {
            ...
            var lista = f?.GetValue(null) as System.Collections.IEnumerable;
            if (lista == null)
                return null;
            ...
            {
                var t = it.GetType();
                var n = t.GetProperty("Nombre")?.GetValue(it)?.ToString();
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(rzCsv))
                {
                    foreach (var r in rzCsv.Split(','))
                    {
                        var sLoc = NormalizarRarezaTextoLocal(r.Trim());
                        candidatas.Add(MiJuegoRPG.Objetos.RarezaHelper.Normalizar(sLoc));
                    }
                }

                if (candidatas.Count == 0)
                {
                    candidatas.Add(MiJuegoRPG.Objetos.RarezaHelper.Normalizar(rzStr));
                }
                ...
                {
                    pmin = Math.Max(pmin, pMin.Value);
                    pmax = Math.Min(pmax, pMax.Value);
                    if (pmin > pmax)
                    {
                        pmin = rango.Item1;
                        pmax = rango.Item2;
                ...
                {
                    nivel = new Random().Next(Math.Max(1, nMin.Value), Math.Max(nMin.Value, nMax.Value) + 1);
                }
                if (dMin.HasValue && dMax.HasValue)
                {
                    danio = new Random().Next(Math.Max(0, dMin.Value), Math.Max(dMin.Value, dMax.Value) + 1);
                }
        ...
        private MiJuegoRPG.Objetos.Armadura? CrearArmaduraDesdeNombre(string nombre)
        {
            ...
            try
            {
                ...
                if (dMin.HasValue && dMax.HasValue)
                {
                    def = new Random().Next(Math.Max(0, dMin.Value), Math.Max(dMin.Value, dMax.Value) + 1);
                }
        ...
        private MiJuegoRPG.Objetos.Botas? CrearBotasDesdeNombre(string nombre)
        {
            ...
            var lista = t?.GetValue(null) as System.Collections.IEnumerable;
            if (lista == null)
                return null;
            object? baseData = null;
            foreach (var it in lista)
            {
                var p = it.GetType().GetProperty("Nombre");
                var v = p?.GetValue(it)?.ToString();
                if (string.Equals(v, nombre, StringComparison.OrdinalIgnoreCase))
                {
                    baseData = it;
                    break;
                }
        ...
        private MiJuegoRPG.Objetos.Cinturon? CrearCinturonDesdeNombre(string nombre)
        {
            ...
            try
            {
                ...
                if (cMin.HasValue && cMax.HasValue)
                {
                    carga = new Random().Next(Math.Max(0, cMin.Value), Math.Max(cMin.Value, cMax.Value) + 1);
                }
        ...
        private MiJuegoRPG.Objetos.Collar? CrearCollarDesdeNombre(string nombre)
        {
            ...
            try
            {
                ...
                if (bonDefMin.HasValue && bonDefMax.HasValue)
                {
                    bonDef = new Random().Next(Math.Max(0, bonDefMin.Value), Math.Max(bonDefMin.Value, bonDefMax.Value) + 1);
                }
                if (bonEneMin.HasValue && bonEneMax.HasValue)
                {
                    bonEne = new Random().Next(Math.Max(0, bonEneMin.Value), Math.Max(bonEneMin.Value, bonEneMax.Value) + 1);
                }
                int bonDefFinal = (int)Math.Round(bonDef * (perfeccion / 50.0), MidpointRounding.AwayFromZero);
        ...
        private void VerDropsUnicos()
        {
            try
            {
                ...
                foreach (var k in keys)
                {
                    if (i++ >= maxMostrar)
                    {
                        juego.Ui.WriteLine($"... (+{n - maxMostrar} más)");
                        break;
                    }
        ...
        private void CambiarClaseActiva()
        {
            ...
            {
                if (idx >= 1 && idx <= lista.Count)
                    seleccion = lista[idx - 1];
            }
            else
            {
        ...
        private void ListarCooldownsEncuentros()
        {
            ...
            {
                juego.Ui.WriteLine("[ADMIN] EncuentrosService no inicializado.");
                return;
            }
            var estados = juego.EncuentrosService.ObtenerEstadoCooldowns();
            if (estados.Count == 0)
            {
                juego.Ui.WriteLine("[ADMIN] No hay cooldowns registrados.");
                return;
            }
        ...
        private void AjustarNivelAdmin(Personaje.Personaje pj, int delta)
        {
            if (delta == 0)
            {
                juego.Ui.WriteLine("Delta 0, sin cambios.");
                return;
            }
            ...
            if (nuevo < 1)
                nuevo = 1;
            if (nuevo > 500)
                nuevo = 500; // límite arbitrario
            if (nuevo == pj.Nivel)
            {
                juego.Ui.WriteLine("Nivel sin cambio efectivo.");
                return;
            }
            if (nuevo > pj.Nivel)
            {
        ...
        private bool ModificarAtributosBatch(Personaje.Personaje pj, string entrada)
        {
            if (string.IsNullOrWhiteSpace(entrada))
                return false;
            int aplicados = 0;
            var desconocidos = new System.Collections.Generic.List<string>();
            ...
            foreach (var raw in partes)
            {
                ...
                if (idxMas > 0)
                {
                    ...
                    if (!double.TryParse(num, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out delta))
                    {
                        desconocidos.Add(t);
                        continue;
                    }
                ...
                if (ModificarAtributo(pj, attr, delta))
                    aplicados++;
                else
                    desconocidos.Add(attr);
            }
            if (aplicados > 0)
            {
                ClampAtributos(pj);
        ...
        private void ListarClases()
        {
            ...
            if (pj.ClasesDesbloqueadas.Count == 0)
            {
                juego.Ui.WriteLine("(ninguna)");
            }
            else
            {
                foreach (var c in pj.ClasesDesbloqueadas.OrderBy(c => c))
                    juego.Ui.WriteLine("- " + c);
            }
            ...
            if (campoDefs == null)
            {
                juego.Ui.WriteLine("No se pudieron obtener definiciones de clases.");
                return;
            }
        ...
        private List<string> MotivosBloqueoClase(Personaje.Personaje pj, object def)
        {
            ...
            string PropStr(string n)
            {
                try
                {
                    return def?.GetType()?.GetProperty(n)?.GetValue(def) as string ?? string.Empty;
                }
            ...
            int PropInt(string n)
            {
                try
                {
                    var v = def?.GetType()?.GetProperty(n)?.GetValue(def);
            ...
            if (propExcl != null)
            {
                foreach (var ex in propExcl)
                {
                    if (ex == null)
                        continue;
                    if (pj.TieneClase(ex.ToString()!))
                        razones.Add("Excluida por: " + ex);
                }
            }
            // Clases previas
            if (propClasesPrev != null)
            {
                ...
                foreach (var c in propClasesPrev)
                {
                    if (c == null)
                        continue;
                    if (!pj.TieneClase(c.ToString()!))
                        faltan.Add(c.ToString()!);
                }
                if (faltan.Count > 0)
                    razones.Add("Faltan clases previas: " + string.Join(", ", faltan));
            }
            // Clases alguna
            if (propClasesAlguna != null)
            {
                ...
                if (!tieneAlguna && lista.Count > 0)
                    razones.Add("Requiere alguna de: " + string.Join(", ", lista));
            }
            if (nivelMin > 0 && pj != null && pj.Nivel < nivelMin)
                razones.Add($"Nivel {pj?.Nivel ?? 0}/{nivelMin}");
            if (repMin > 0 && pj != null && pj.Reputacion < repMin)
                razones.Add($"Reputación {pj?.Reputacion ?? 0}/{repMin}");
            if (propRepFac != null && pj != null && pj.ReputacionesFaccion != null)
            {
                foreach (System.Collections.DictionaryEntry kv in propRepFac)
                {
                    ...
                    if (v < req)
                        razones.Add($"Reputación {kv.Key} {v}/{req}");
                }
            }
            if (!string.IsNullOrWhiteSpace(misionUnica) && pj != null && pj.MisionesCompletadas != null && !pj.MisionesCompletadas.Any(m => (m?.Id ?? "").Equals(misionUnica, StringComparison.OrdinalIgnoreCase) || (m?.Nombre ?? "").Equals(misionUnica, StringComparison.OrdinalIgnoreCase)))
                razones.Add("Misión única no completada: " + misionUnica);
            if (!string.IsNullOrWhiteSpace(objetoUnico) && pj != null && pj.Inventario != null && pj.Inventario.NuevosObjetos != null && !pj.Inventario.NuevosObjetos.Any(o => o?.Objeto?.Nombre != null && o.Objeto.Nombre.Equals(objetoUnico, StringComparison.OrdinalIgnoreCase)))
                razones.Add("Objeto único faltante: " + objetoUnico);
            if (propMis != null && pj != null && pj.MisionesCompletadas != null)
            {
                ...
                foreach (var m in propMis)
                {
                    if (m == null)
                        continue;
                    if (!pj.MisionesCompletadas.Any(x => (x?.Id ?? "").Equals(m.ToString(), StringComparison.OrdinalIgnoreCase) || (x?.Nombre ?? "").Equals(m.ToString(), StringComparison.OrdinalIgnoreCase)))
                        faltanMis.Add(m.ToString()!);
                }
                if (faltanMis.Count > 0)
                    razones.Add("Misiones pendientes: " + string.Join(", ", faltanMis));
            }
            ...
            if (propAct != null && pj != null && pj.ContadoresActividad != null)
            {
                foreach (System.Collections.DictionaryEntry kv in propAct)
                {
                    ...
                    int req = kv.Value is int i ? i : 0;
                    if (v < req)
                        razones.Add($"Actividad {kv.Key} {v}/{req}");
                }
            }
            if (propStats != null && pj != null)
            {
                foreach (System.Collections.DictionaryEntry kv in propStats)
                {
                    ...
                    int req = kv.Value is int i ? i : 0;
                    if (val < req)
                        razones.Add($"Stat {kv.Key} {val:F1}/{req}");
                }
            }
            return razones;
        }
        ...
        private void ListarInventarioYEquipo()
        {
            ...
            if (pj.Inventario.NuevosObjetos.Count == 0)
            {
                juego.Ui.WriteLine("(vacío)");
            }
            else
            {
                foreach (var o in pj.Inventario.NuevosObjetos)
                    juego.Ui.WriteLine($"- {o.Objeto.Nombre} x{o.Cantidad}");
            }

            juego.Ui.WriteLine("=== Equipo ===");
            var eq = pj.Inventario.Equipo;
            void Imprimir(string label, object? item)
            {
                juego.Ui.WriteLine(label + ": " + (item == null ? "(libre)" : ((MiJuegoRPG.Objetos.Objeto)item).Nombre));
            }
            Imprimir("Arma", eq.Arma);
            Imprimir("Casco", eq.Casco);
            Imprimir("Armadura", eq.Armadura);
            Imprimir("Pantalon", eq.Pantalon);
            Imprimir("Botas", eq.Zapatos);
            Imprimir("Collar", eq.Collar);
            Imprimir("Cinturon", eq.Cinturon);
            Imprimir("Accesorio1", eq.Accesorio1);
            Imprimir("Accesorio2", eq.Accesorio2);
        }
        ...
        private void ResumenIntegral()
        {
            ...
            if (juego.Jugador != null)
            {
                ...
                foreach (var kv in juego.Jugador.ReputacionesFaccion.OrderBy(k => k.Key))
                    juego.Ui.WriteLine($"{kv.Key}: {kv.Value}");
            }
        }
        ...
        private void ForzarClase()
        {
            ...
            if (string.IsNullOrWhiteSpace(nombre))
            {
                juego.Ui.WriteLine("Nombre vacío.");
                return;
            }
            ...
            if (campoDefs == null)
            {
                juego.Ui.WriteLine("No se pudieron leer definiciones de clases.");
                return;
            }
            var defs = campoDefs.GetValue(juego.ClaseService) as System.Collections.IEnumerable;
            if (defs == null)
            {
                juego.Ui.WriteLine("Sin definiciones cargadas.");
                return;
            }
            ...
            if (pj.TieneClase(nombre))
            {
                juego.Ui.WriteLine("Ya tenías esta clase.");
                return;
            }
            ...
            if (pj.DesbloquearClase(nombre))
            {
                try
                {
                    ...
                    try
                    {
                        if (juego.ClaseService.Evaluar(pj))
                            juego.Ui.WriteLine("[CLASES] Desbloqueos adicionales tras forzar clase.");
                    }
                    catch { }
        ...
        private void ExportarResumenIntegral()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        var h = kv.Value;
                        sb.AppendLine($"- {h.Nombre} (ID {h.Id}) Nv {h.Nivel} Exp {h.Exp}");
                    }
                }
                ...
                {
                    foreach (var o in pj.Inventario.NuevosObjetos)
                        sb.AppendLine($"- {o.Objeto.Nombre} x{o.Cantidad}");
                }

                sb.AppendLine("=== Equipo ===");
                var eq = pj.Inventario.Equipo;
                string Nom(object? item) => item == null ? "(libre)" : ((MiJuegoRPG.Objetos.Objeto)item).Nombre;
                sb.AppendLine($"Arma: {Nom(eq.Arma)}");
                sb.AppendLine($"Casco: {Nom(eq.Casco)}");
                sb.AppendLine($"Armadura: {Nom(eq.Armadura)}");
                sb.AppendLine($"Pantalon: {Nom(eq.Pantalon)}");
                sb.AppendLine($"Botas: {Nom(eq.Zapatos)}");
                sb.AppendLine($"Collar: {Nom(eq.Collar)}");
                sb.AppendLine($"Cinturon: {Nom(eq.Cinturon)}");
                sb.AppendLine($"Accesorio1: {Nom(eq.Accesorio1)}");
                sb.AppendLine($"Accesorio2: {Nom(eq.Accesorio2)}");

                // Reputación
                sb.AppendLine();
                sb.AppendLine("=== Reputación ===");
                sb.AppendLine($"Global: {pj.Reputacion}");
                foreach (var kv in pj.ReputacionesFaccion.OrderBy(k => k.Key))
        ...
        private void AjustarTiempoDelMundo()
        {
            ...
            {
                juego.Ui.WriteLine("Sin cambios.");
                return;
            }
            if (val.StartsWith("h=", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    juego.Ui.WriteLine("Formato inválido. Ejemplos válidos: h=20, h=5");
                }
            ...
            {
                juego.Ui.WriteLine("Valor inválido. Usa +N, -N o h=HH.");
            }
        }
    }
}
