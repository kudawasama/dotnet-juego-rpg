...
namespace MiJuegoRPG.Motor.Servicios
{
    ...
    public class AccionRegistry
    {
        ...
        {
            if (pj == null || string.IsNullOrWhiteSpace(accionId))
                return false;
            ...
            foreach (var h in habilidadesConAccion)
            {
                ...
                if (pj.Habilidades != null && pj.Habilidades.ContainsKey(h.Id))
                    continue;
        ...
        {
            if (pj?.ProgresoAccionesPorHabilidad == null)
                return 0;
            if (!pj.ProgresoAccionesPorHabilidad.TryGetValue(habilidadId, out var mapa))
                return 0;
