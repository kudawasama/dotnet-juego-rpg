...
namespace MiJuegoRPG.Motor.Servicios.Repos
{
    ...
    {
        private readonly Dictionary<string, PantalonData> cache = new(StringComparer.OrdinalIgnoreCase);
        private bool loaded;
        ...
        {
            EnsureLoaded();
            return cache.Values as IReadOnlyCollection<PantalonData> ?? new List<PantalonData>(cache.Values);
        }
