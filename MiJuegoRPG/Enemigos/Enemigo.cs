...
namespace MiJuegoRPG.Enemigos
{
    ...
    public abstract class Enemigo : ICombatiente, IEvadible
    {
        ...
        public const double BASEDROPRATE = 0.05; // 5% base
        public const double RAREZAMULTIPLIERRota = 0.1;
        public const double RAREZAMULTIPLIERPobre = 0.3;
        public const double RAREZAMULTIPLIERNormal = 0.5;
        public const double RAREZAMULTIPLIERSuperior = 0.7;
        public const double RAREZAMULTIPLIERRara = 0.85;
        public const double RAREZAMULTIPLIERLegendaria = 0.95;
        public const double RAREZAMULTIPLIEROrnamentada = 1.0;
        ...
        public void AgregarDanioElementalBase(string tipo, int cantidad)
        {
            if (string.IsNullOrWhiteSpace(tipo))
                return;
            if (DanioElementalBase.ContainsKey(tipo))
                DanioElementalBase[tipo] += Math.Max(0, cantidad);
            else
                DanioElementalBase[tipo] = Math.Max(0, cantidad);
        }
            ...
            if (!EstaVivo)
            {
                ...
                if (drop != null)
                {
                    ...
                    if (cant > 0)
                    {
                        ...
                        try
                        {
                            jugador.Inventario.AgregarObjeto(drop, cant, jugador);
                        }
                        catch { }
                    }
                }
